...
# ==============================================================================
echo -e "${CYAN}[1/4] Verificando dependências...${NC}"
...
if ! command -v aws &>/dev/null; then
    echo -e "${YELLOW}AVISO: AWS CLI não encontrado. O certificado será configurado, mas o AWS CLI precisará ser instalado separadamente.${NC}"
else
...
# ==============================================================================
echo -e "${CYAN}[2/4] Baixando certificado Zscaler...${NC}"
...
if [ -f "$CAMINHO_CERTIFICADO_LOCAL" ]; then
    ...
    if [[ ! "$overwrite" =~ ^[sS]$ ]]; then
        ...
        if curl --fail -s -L "$URL_CERTIFICADO" -o "$CAMINHO_CERTIFICADO_LOCAL"; then
            ...
            echo -e "${RED}ERRO: Falha ao baixar o certificado. Verifique sua conexão e tente novamente.${NC}"
            exit 1
        fi
    fi
else
...
# ==============================================================================
echo -e "${CYAN}[3/4] Persistindo AWS_CA_BUNDLE nos arquivos de perfil...${NC}"
...
# ==============================================================================
echo -e "${CYAN}[4/4] Validando configuração...${NC}"

# Verificar variável na sessão atual
if [ -n "$AWS_CA_BUNDLE" ]; then
    echo -e "${GREEN}  -> AWS_CA_BUNDLE está definido: $AWS_CA_BUNDLE${NC}"
else
...
if [ -s "$CAMINHO_CERTIFICADO_LOCAL" ]; then
    echo -e "${GREEN}  -> Arquivo de certificado existe e não está vazio.${NC}"
else
    echo -e "${RED}  -> ERRO: Arquivo de certificado não encontrado ou está vazio.${NC}"
fi

# Testar conectividade com a AWS (se AWS CLI estiver disponível)
if command -v aws &>/dev/null; then
    echo -e "  -> Testando conectividade com a AWS..."
    if aws sts get-caller-identity &>/dev/null; then
        echo -e "${GREEN}  -> Conexão com AWS OK! Sessão ativa encontrada.${NC}"
    else
        echo -e "${YELLOW}  -> Sem sessão AWS ativa (normal para novo usuário). Execute 'aws sso login' para autenticar.${NC}"
    fi
fi
...
echo -e "  2. Autentique-se na AWS:"
echo -e "     ${YELLOW}aws sso login --profile <seu-perfil>${NC}"
echo ""
